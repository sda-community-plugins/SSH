...
try {
    ...
    String localFileName = props.notNull('localFile')
    ...
    if (verbose) {
        println ">>> Uploading file ${localFileName} into ${remoteDirName}..."
    }
    try {
        remoteSession {
            connect()
            scp {
                into { remoteDir(remoteDirName) }
                from { localFile(localFileName) }
            }
            disconnect()
        }
